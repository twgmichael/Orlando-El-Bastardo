...
| `ships/jb100_v38/jb100_v38.glb` (+`.blend`, `.usdc`, canonical aliases `ships/jb100.glb`/`.usdc`) | Built by `tools/build_jb100.py` — fork of the JB5K reconstruction; the JourneyBlaster 100, first version of the Yakara Starcraft sport-attack line; cockpit set approved and promoted 2026-08-24 | Owner's original design; project original | 1 (no external license) | Canonical node `prop_jb100_A`; v38 production role is pilot-free hero stand-in. Orlando remains a separately cast/animated actor and is never baked into the ship asset. |
