#!/usr/bin/env python3
"""
build_jb5k.py — Headless Blender: the JourneyBlaster 5000 from primitives.

Reconstruction of the owner's 1995 Infini-D "Journey Blaster 5000" sport
attack craft (Yakara Starcraft) from the 2000/2001 reference renders
...
top shell's HOLLOW centre, holding TWO
simple L-chairs IN LINE (tandem, per the reference); bowl and bubble
...
Run from repo root:
  blender --background --factory-startup \
    --python tools/build_jb5k.py -- --output assets/ships/jb5k
"""
...
def parse_args():
    ...
    p = argparse.ArgumentParser(prog="build_jb5k")
    p.add_argument("--output", default="assets/ships/jb5k")
...
def main():
    ...
    mat_gold = mat("mat_jb5k_intake", (0.85, 0.62, 0.12), rough=0.25,
                   metallic=0.9)
    mat_lamp = mat("mat_jb5k_lamp", (1.0, 0.5, 0.03), rough=0.15,
                   emission=((1.0, 0.32, 0.01), 3.2))
    mat_burn = mat("mat_jb5k_exhaust", (0.35, 0.02, 0.02), rough=0.4,
                   emission=((0.85, 0.04, 0.02), 0.8))
    mat_dark = mat("mat_jb5k_interior", (0.08, 0.08, 0.1), rough=0.8)
    mat_white = mat("mat_jb5k_disc", (0.92, 0.92, 0.95), rough=0.35,
                    emission=((1.0, 1.0, 1.0), 0.3))
    mat_seat = mat("mat_jb5k_seat", (0.34, 0.35, 0.4), rough=0.6)
    ...
    rb = hull_b - LIP
    bm = bmesh.new()
    profile = [(0.02, 0.0), (rb - BOT_FILLET, 0.0)]
    for k in range(1, 5):
        a = math.pi / 2 * k / 4
    ...
    bowl_bottom = obj_from_bmesh("jb5k_hull_bottom", bm, mat_hull)
    ...
    bm = bmesh.new()
    for cy in SEAT_YS:
        add_box(bm, (0.42, 0.44, 0.08), (0, cy, 0.42))       # seat of the L
        add_box(bm, (0.42, 0.08, 0.55), (0, cy + 0.24, 0.72))  # back of the L
    add_box(bm, (0.6, 0.24, 0.2), (0, -0.78, 0.52))          # console
    seats = obj_from_bmesh("jb5k_seats", bm, mat_seat)
    ...
    lamps = obj_from_bmesh("jb5k_lamps", bm, mat_lamp)
    ...
    for sx in (-1, 1):
        ...
        for v in res["verts"]:
            co = Vector((v.co.x * POD_R, v.co.y * POD_L * 0.5,
                         v.co.z * POD_R))
            v.co = yaw @ co + centre
        pod = obj_from_bmesh(f"jb5k_pod_{'l' if sx < 0 else 'r'}",
                             bm, mat_pod)
        pod_objs.append(pod)
        ...
        add_cone(bm, POD_R * 0.62, POD_R * 0.5, 0.1,
                 ...
                 segs=16)                                      # red core
        burn = obj_from_bmesh(f"jb5k_exhaust_{'l' if sx < 0 else 'r'}",
                              bm, mat_burn)
    ...
    for k in range(DISC_N):
        ...
        add_cone(bm, DISC_R, DISC_R, 0.08, (dx, dy, 0.03), segs=18)
        add_cone(bmt, THRUSTER_R, THRUSTER_R * 0.55, 0.1,
                 (dx, dy, -0.02), segs=10)   # stub protrudes below
    discs = obj_from_bmesh("jb5k_belly_discs", bm, mat_white)
    pod_objs.append(discs)
    thrusters = obj_from_bmesh("jb5k_belly_thrusters", bmt, mat_pod)
    ...
    ship.name = "prop_jb5k_A"
    ship.data.name = "prop_jb5k_A_mesh"
    print(f"[build_jb5k] joined: {len(ship.data.polygons)} polys, "
          f"{len(ship.material_slots)} materials")

    glb = out_stem + ".glb"
    print(f"[build_jb5k] Exporting {glb}")
    bpy.ops.export_scene.gltf(filepath=glb, export_format='GLB',
                              use_selection=False)
    usdc = out_stem + ".usdc"
    print(f"[build_jb5k] Exporting {usdc}")
    try:
        bpy.ops.wm.usd_export(filepath=usdc, export_animation=False,
                              export_materials=True)
    except TypeError:
        bpy.ops.wm.usd_export(filepath=usdc)
    print("[build_jb5k] Done.")
